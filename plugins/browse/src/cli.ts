...
async function initBrowser(): Promise<{ browser: Browser; context: BrowserContext; page: Page }> {
  ...
  if (chromeReady) {
    ...
    if (chromeProcess.pid) {
      ...
      writeFileSync(
        pidFilePath,
        JSON.stringify({ pid: chromeProcess.pid, startTime: Date.now() }),
      );
    }
...
async function cmdScroll(direction: string, amount?: number) {
  const { page } = await initBrowser();
  const px = amount ?? 500;
...
async function cmdEvaluate(script: string) {
  const { page } = await initBrowser();
  const result = await page.evaluate(script);
  return { success: true, result: result ?? null };
}

async function cmdSelect(selector: string, value: string) {
  const { page } = await initBrowser();
...
async function cmdWait(selector?: string, timeout?: number) {
  const { page } = await initBrowser();
...
async function main() {
  ...
  try {
    ...
    switch (command) {
      case 'navigate':
        if (!args[1]) throw new Error('Usage: cli.js navigate <url>');
        result = await cmdNavigate(args[1]);
        break;

      case 'click':
        if (!args[1]) throw new Error('Usage: cli.js click <selector>');
        result = await cmdClick(args[1]);
        break;

      case 'type':
        if (!args[1] || !args[2]) throw new Error('Usage: cli.js type <selector> <text>');
      ...
      case 'scroll': {
        if (!args[1]) throw new Error('Usage: cli.js scroll <direction> [amount]');
      ...
      case 'evaluate':
        if (!args[1]) throw new Error('Usage: cli.js evaluate <script>');
        result = await cmdEvaluate(args[1]);
        break;

      case 'select':
        if (!args[1] || !args[2]) throw new Error('Usage: cli.js select <selector> <value>');
