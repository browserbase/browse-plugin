...
async function getPage() {
    ...
    if (chromeReady) {
        ...
        _chromeProcess = spawn(chromePath, [
        ...
        if (_chromeProcess.pid) {
            ...
            writeFileSync(pidFilePath, JSON.stringify({ pid: _chromeProcess.pid, startTime: Date.now() }));
        }
        for (let i = 0; i < 50; i++) {
...
async function safeTool(fn) {
    try {
        ...
        return json({ success: false, error: err instanceof Error ? err.message : String(err) }, true);
    }
}
// ---------- MCP Server ----------
...
server.registerTool('browser_navigate', {
    description: 'Navigate to a URL in the browser. Launches Chrome automatically on first call. Returns page title and a screenshot file path.',
    inputSchema: {
        url: z.string().describe('The URL to navigate to (include https://)'),
    },
}, async ({ url }) => safeTool(async () => {
...
server.registerTool('browser_click', {
    description: 'Click an element on the page by CSS selector. Use browser_snapshot first to find the right selector.',
    inputSchema: {
        selector: z.string().describe('CSS selector of the element to click'),
    },
}, async ({ selector }) => safeTool(async () => {
    const page = await getPage();
...
server.registerTool('browser_type', {
    description: 'Clear an input field and type text into it. Use browser_snapshot to find the selector for the input.',
    inputSchema: {
        selector: z.string().describe('CSS selector of the input element'),
        text: z.string().describe('Text to type into the field'),
    },
}, async ({ selector, text }) => safeTool(async () => {
    const page = await getPage();
    await page.click(selector, { timeout: 10_000 });
...
server.registerTool('browser_snapshot', {
    description: 'Get all interactive elements on the page with their CSS selectors, roles, text, and state. Use this to find the right selector before clicking or typing. Returns elements like buttons, links, inputs, selects, and textareas.',
    inputSchema: {},
...
}, async ({ direction, amount }) => safeTool(async () => {
    const page = await getPage();
    const px = amount ?? 500;
...
server.registerTool('browser_evaluate', {
    description: 'Run arbitrary JavaScript in the browser page context. Returns the result as a string. Useful for extracting specific data or performing complex interactions.',
    inputSchema: {
        script: z.string().describe('JavaScript code to execute in the page context'),
    },
}, async ({ script }) => safeTool(async () => {
    const page = await getPage();
...
server.registerTool('browser_select', {
    description: 'Select an option in a <select> dropdown by its value. Returns a screenshot.',
    inputSchema: {
        selector: z.string().describe('CSS selector of the <select> element'),
        value: z.string().describe('Value of the option to select'),
    },
}, async ({ selector, value }) => safeTool(async () => {
    const page = await getPage();
...
server.registerTool('browser_wait', {
    description: 'Wait for an element to appear on the page, or wait a fixed number of milliseconds. Useful after navigation or actions that trigger page changes.',
    inputSchema: {
        selector: z
            .string()
            .optional()
...
}, async ({ selector, timeout }) => safeTool(async () => {
    const page = await getPage();
...
server.registerTool('browser_close', {
    description: 'Close the browser and free resources. Call this when done with browser automation.',
    inputSchema: {},
}, async () => safeTool(async () => {
    await closeBrowser();
