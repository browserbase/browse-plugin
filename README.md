...
Each per-format `plugin.json` references `./skills/` and the logo relative to repo root, and each root marketplace file's `source` points at `.` — no nested `plugins/<name>/` indirection. A nested `plugins/<name>/` layout breaks third-party tools (e.g. Hermes Agent's skill "tap") that scan for `skills/` at repo root by convention, so this repo stays flat.
